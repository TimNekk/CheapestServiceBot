...
prolong_categories_callback_data = CallbackData("prolong_category", "category_id", "number", "id")
...
def make_prolong_categories_callback_data(category_id: int, number: str, id: str):
    return prolong_categories_callback_data.new(category_id=category_id, number=number, id=id)
...
def prolong_categories_keyboard(service_id: int, number: str, id: str) -> InlineKeyboardMarkup:
    keyboard = InlineKeyboardMarkup()

    service = db.get_service(service_id)
    categories = service.get_categories(show_only=False)
    for category in categories:
        keyboard.add(InlineKeyboardButton(text=category.name, callback_data=make_prolong_categories_callback_data(category.id, number, id)))

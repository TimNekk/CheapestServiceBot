...
class Database:
    ...
    def get_users_count_by_date(self, filter_date: date) -> int:
        date_str = filter_date.strftime('%Y-%m-%d')
        sql = f"SELECT COUNT(*) FROM Users WHERE DATETIME(join_datetime) > '{date_str} 00:00:00' AND DATETIME(join_datetime) < '{date_str} 23:59:59'"
        return self.execute(sql, fetchone=True)[0]
